...
import com.slp.service.AsyncThreadPool;
import com.slp.service.FastefsClient;
import cpm.slp.domain.Picture;
import cpm.slp.model.PictureQo;
import cpm.slp.service.PictureService;
...
public class PicUtilController {
    ...
    public void uploadPic(@RequestParam("pictureFile") MultipartFile multipartFile, HttpServletRequest request, HttpServletResponse response) {
        try {
            String filename = fastefsClient.uploadFile(multipartFile);
            Long shopid = 1L;

            /**
             * 使用后台线程执行保存的方法，将不会影响界面上操作
             */
    ...
    public void cutPic(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String newfile = fastefsClient.uploadFile(is, size, imageType);
    ...
    public void uploadimg(@RequestParam("upfile") MultipartFile upfile,HttpServletRequest request,HttpServletResponse response){
        try {
            String filename = fastefsClient.uploadFile(upfile);
